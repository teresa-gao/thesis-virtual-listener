...
from face_util import draw_landmarks
...
def run_empathizer_neutral(prompt, total_seconds=90, frame_delay=0, frame_repeats=5):
    ...
    while time() - start < total_seconds:
        ...
        if i <= 0 or i >= len(neutral_landmarks) - 1:
            i_delta *= -1
        i += i_delta
        draw_landmarks(frame, landmarks=neutral_landmarks[floor(i)])
